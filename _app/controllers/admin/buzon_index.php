...
if (!admin_is_async_request()) {
    $pageTitle = 'Buzón del Rector | Admin CDV';
...
try {
    ...
    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM buzon_rector_messages' . $whereSql);
    ...
    $stmt = $pdo->prepare('SELECT * FROM buzon_rector_messages' . $whereSql . ' ORDER BY created_at DESC LIMIT :limit OFFSET :offset');
    ...
    $dbError = 'No se pudo cargar la tabla de buzón. Revisa la configuración de BD.';
}
