...
async function optimizeWebpInPlace(filePath) {
  ...
  await sharp(filePath, { failOn: 'none' }).webp({ quality: 82 }).toFile(tmp);
  fs.renameSync(tmp, filePath);
}

async function generateWebpFromSource(sourcePath, targetWebp) {
  const tmp = `${targetWebp}.tmp`;
  await sharp(sourcePath, { failOn: 'none' }).webp({ quality: 82 }).toFile(tmp);
...
function cleanupManifestEntries(manifest) {
...
(async () => {
  ...
  for (const task of tasks) {
    try {
      ...
      if (task.type === 'convert-to-webp') {
        ...
        // Nunca eliminar origen hasta validar que el .webp existe y es legible.
        if (!fs.existsSync(task.targetWebp)) {
          throw new Error('No se genero el archivo .webp de salida');
        }

        const outStats = fs.statSync(task.targetWebp);
        if (outStats.size <= 0) {
          throw new Error('El .webp generado esta vacio');
        }
  ...
  cleanupManifestEntries(manifest);
  saveManifest(manifest);

  console.log('--- Resumen ---');
  console.log(`Tareas detectadas: ${tasks.length}`);
